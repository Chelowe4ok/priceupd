...
function downloadDataFromWebSiteFrankeStore(option, http){
          ...
          function callbackData(res) {
              ...
              res.on('end', function () {
                  ...
                  console.log( "DOWNLOAD DATA FROM FrankeStore!!!!");
                  console.log("option-" + mdvdPrice.tmpcounter + ": " + option.host + option.path + ". Product_id: " + option.product_id);
                mdvdPrice.tmpcounter++;
                  
                let regularExpPath = /<link\shref="https:\/\/www\.frankestore\.com\.ua(.*?)"\srel="canonical"/
                    ...
                    if ( resultNotFound[1] == "Запрашиваемая страница не найдена!"  || resultNotFound[1] == "Доступ запрещен"){
						...
						mdvdPrice.connection.query('UPDATE mdvdoc_product SET quantity = 0, stock_status_id = 5 WHERE jan = ?', option.path, function(err, result) {
							...
							if (!err){
								...
								console.log('Error while performing Query in update not fined products in franke!');
							}
...
//                        mdvdPrice.mapTmp.push( resultPath );
                ...
                let regularExpPrice;
                  if (/<h2\sclass="product-price">(.*?)\sгрн.<\/h2>/.test(data)){
                      regularExpPrice = /<h2\sclass="product-price">(.*?)\sгрн.<\/h2>/;
                  }else if ( /special-price">(.*?)\sгрн.<\/h2>/.test(data) ){
                      regularExpPrice = /special-price">(.*?)\sгрн.<\/h2>/;
                  }else{
                      regularExpPrice = /product-price">(.*?)\sгрн.<\/h2>/;
                  }
                ...
                console.log("data: " + /<link\shref="https:\/\/www\.frankestore\.com\.ua(.*?)"\srel="canonical"/.test(data));
                mdvdPrice.dataMap.set(mdvdPrice.prod_id, { path: resultPath, price: resultPrice, exchangeRates: null, manufacturer: "Franke", stockStatus: null, status: null, notFound: resultNotFound });
...
module.exports = downloadDataFromWebSiteFrankeStore;